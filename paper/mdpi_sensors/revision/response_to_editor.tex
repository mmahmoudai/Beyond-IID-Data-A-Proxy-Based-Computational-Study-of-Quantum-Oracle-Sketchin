...
\emph{Response.} We thank the Academic Editor and agree with the
...
entry, which now reads \emph{Information Sciences} 676, article 120799,
...
publisher page; and all seven authors' publication pages give only the
arXiv posting. We retain it because it is not supporting evidence for a
claim of ours --- it is the object of study. The paper examines how that
...
and a footnote at its first citation states this status and why no
conclusion depends on its final form: no quantum algorithm is
implemented or simulated, and every inherited quantity is listed with
its provenance and epistemic status in Table 1. We also added the
strongest published anchor of its kind --- Kallaugher, Parekh and
Voronova (STOC 2024, pp.~1805--1815, DOI 10.1145/3618260.3649709), the
first exponential quantum--classical space separation for a natural
streaming problem --- co-cited with it in the Related Work and in the
footnote; in doing so we corrected a Related Work sentence that had
credited the 2021 FOCS paper with an exponential separation (its
advantage is polynomial). The bibliography now has 55 entries, and we
re-checked that every in-text citation matches exactly one entry.
