...
\Title{Quantum Oracle Sketching for Non-IID Sensor and Telemetry Streams: A Proxy-Based Computational Study}
\TitleCitation{Quantum Oracle Sketching for Non-IID Sensor and Telemetry Streams}
...
Panel~(a) shows that at $n = 14$ ($N = 16{,}384$) the representative classical memory reference exceeds 128 while the quantum requirement is only 14 qubits — an exponential gap that widens with $n$ (the $\Omega(\sqrt{N})$ floor is a worst-case reference, not an empirical floor; see Section~\ref{sec:heuristic_model}). Panel~(b) shows that classical SGD accuracy is approximately stable across $n \in [6, 14]$, while the quantum IID proxy is consistently near $1.0$. This flatness is a property of the task-level $\sqrt{n/\Teff}$ rate; under the exact framework's oracle-construction cost, which grows linearly in $N = 2^n$, the fixed horizon $T = 10^4$ is already comparable to $N$ at $n = 14$, so the flat trend should not be read as construction feasibility at higher dimensions (Section~\ref{sec:heuristic_model}). The quantum Markov proxy \emph{decreases} with dimension because $\tau_{\text{Markov}} = n/(1-\rho)$ grows linearly with $n$; family-specific proxy refinements are therefore most useful at higher dimensions.
...
\begin{enumerate}
    ...
    \item \textbf{Proxy parameters are not formally equivalent to the exact framework's quantities.} Our $\tau$ and $r$ are paper-defined operational labels (Section~\ref{sec:prelim}); equivalence to the exact correlation parameters of \citet{zhao2026exponential} is not proven and is deferred to follow-on work.
    ...
    \item \textbf{Modest dimensions.} Computation is tractable only to $n \leq 14$; compelling memory gaps are expected at $n \geq 20$ per the polynomial-in-$N$ classical bounds, for which $\Omega(\sqrt{N})$ is our conservative reference (Section~\ref{sec:heuristic_model}). The Markov proxy becomes less favourable at higher $n$ because $\tau_{\text{Markov}} = n/(1-\rho)$ grows linearly, making family-specific refinements especially valuable there.
    \item \textbf{Task-specific lower bounds.} The $\Omega(\sqrt{N})$ floor is representative, not universal; exact hardness results in the originating framework are task-specific.
\end{enumerate}

\subsection{Future Work}
\label{sec:future_work}

Future work should pursue: (i) tighter, family-specific correlation characterizations that close the proxy--empirical $\tau$ gap; (ii) extending the two-dataset real-stream sanity check in Section~\ref{sec:real_stream} to financial, network-traffic, and additional sensor streams, characterising their $(\tau, r)$ parameters and mapping them onto Fig.~\ref{fig:landscape} with imbalance-aware metrics throughout; (iii) formal verification of the proxy formulas against the exact framework's correlation parameters; (iv) extension to multiclass classification and regression; and (v) repetition-aware oracle constructions that exploit burst structure variance-reductively rather than treating duplicates as wasted samples.
