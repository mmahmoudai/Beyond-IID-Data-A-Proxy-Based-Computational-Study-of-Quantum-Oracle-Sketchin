...
\item
  ...
  and Quantum Oracle-Sketching Theory''}, separating published theory
  and its boundary / assumptions / heuristic constructions / memory
  references / scope limitations, with a provenance table (Table 1)
  stating per quantity what the cited literature supports, what is
  introduced here, and what is \emph{not} theoretically guaranteed ---
  including the explicit statement that the published quantum-streaming
  results are problem-specific and do not establish the operational τ,
  r, T\_eff or accuracy model used here, so the τ-penalised landscape is
  a classical effective-sample-size stress test used for diagnosis and
...
\item
  ...
  throughout.
\item
  \textbf{Reference audit}: the reference list was audited entry by
  entry and verified against publisher metadata; every DOI was resolved
  and machine-compared against the cited title, first author, venue and
  year. Three outcomes followed. (i) In accordance with the Academic
  Editor's request that preprints not be cited, the Zhao et al.~(2026)
  preprint (arXiv:2604.07639) was removed from the manuscript and the
  ...
  cited for, and the affected statements were instead revised so that
  ...
  (SOSA 2025, pp.~9--45, DOI 10.1137/1.9781611978315.2), and the
  published STOC 2024 result (DOI 10.1145/3618260.3649709) is cited for
  the exponential quantum--classical space separation, which also
  corrects a Related Work sentence that would otherwise have credited
  the 2021 FOCS paper with an exponential separation (the advantage
  established there is polynomial). (iii) A transposed article number in
  the Su, Guo and Wang (2024) entry was corrected (\emph{Information
  Sciences} \textbf{676}, article 120799, DOI
  10.1016/j.ins.2024.120799). The final bibliography contains 54
  references, and every in-text citation was re-checked against it.
\end{enumerate}
...
\emph{Response.} A dedicated subsection now does exactly this, and draws
the boundary explicitly. §3.3 separates (i) \emph{published theory and
boundary} --- the peer-reviewed quantum-streaming results that provide
...
\textbf{not} establish the operational τ, r, T\_eff or accuracy model
used here, and that this paper adds no quantum theorem; (ii)
...
modelling choices of this paper; (iii) \emph{heuristic constructions
introduced in this paper} --- the T\_eff division by τ and r, in the
manner of a classical effective-sample-size correction, and the VC-type
task rate, neither of which is a published quantum guarantee; (iv)
\emph{memory references}, retained solely as paper-defined illustrative
scaling curves; and (v) \emph{scope limitations}. Table 1 tabulates the
provenance and epistemic status of every model-level quantity, with a
``not guaranteed'' column. The relationship is therefore stated as one
of motivation and analogy, not of derivation. \emph{Changes:} New §3.3
...
references'') makes the same statement structurally, and records that
the published quantum-streaming papers do not establish the O(n)/Ω(√N)
pair for the diagnostic task studied here; the provenance table lists
...
column; Figure 6's caption labels panel (a) ``illustrative
memory-scaling reference (paper-defined; no measured content)''; and
§7.1 closes its three structural factors with ``All three factors are
properties of the proxy model or its illustrative references; none is a
...
\emph{Response.} New §3.3 does this separation explicitly, and states
the boundary rather than assuming an equivalence. It has five labelled
parts: \emph{Published theory and boundary} --- the peer-reviewed
...
\textbf{not} establish the operational τ, r, T\_eff or accuracy model
used here, and that this paper adds no quantum theorem;
\emph{Assumptions} (two-scalar summarisability, binarised-stream
fidelity, task-rate-as-pipeline stand-in --- all modelling choices of
this paper); \emph{Heuristic constructions introduced in this paper}
(the T\_eff division by τ and r, following the classical
effective-sample-size correction long standard in Markov-chain output
analysis, and the VC-type task rate --- neither a published quantum
guarantee); \emph{Memory references}, retained solely as paper-defined
illustrative scaling curves; and \emph{Scope limitations}, which states
...
parameter. Table 1 gives per-quantity provenance with an explicit ``not
guaranteed'' column. Directly on the reviewer's question: τ and r are
...
result, and the τ-penalised landscape is presented as a classical
effective-sample-size stress test used for diagnosis and hypothesis
generation. \emph{Changes:} New §3.3 + Table 1; §3.2;
...
\emph{Response.} Made prominent exactly there, and the curves are no
longer presented as cited bounds at all. Experiment 6 now opens with:
``No memory lower bound is measured empirically in this experiment: the
curves in panel (a) are illustrative quantities, and no result here
constitutes an empirical memory floor for online SGD, averaged SGD, or
Count-Min.'' Figure 6's caption labels panel (a) ``illustrative
memory-scaling reference (paper-defined; no measured content)'' and
states that their exponential divergence is a visual scaling contrast,
not an established separation. §3.3's ``Memory references'' paragraph
...
table carries the same status, and §7.1 states that none of its
