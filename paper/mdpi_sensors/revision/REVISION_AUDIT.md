...
**Provenance note (updated 2026-08-11).** The verbatim reviewer reports were received after the first revision pass (R1: 15 recommendations, review dated 07 Aug 2026, "must be improved" on Methods; R2: long-form "extensive revision"). `response_to_reviewers.md` is now the final point-by-point letter with verbatim quotes; it is also issued as two self-contained per-reviewer letters, `response_to_reviewer_1.md` (15 points) and `response_to_reviewer_2.md` (10 points), for submission systems that collect one response file per reviewer. All three carry the manuscript ID sensors-4470240 and no date, share the same header, summary-of-changes block and verification note, and reproduce every point-by-point response verbatim; the only wording difference is in R1.10, where the combined letter's phrase "the look-ahead point raised by Reviewer 2" becomes "the full-series threshold look-ahead raised in review" so the Reviewer-1 letter stands alone. The reports confirmed the brief's digest almost exactly; four points required *new* text beyond the first pass and were added in the third commit: R1.7 (baseline justification, §5.2), R1.9 (regime realism, §4.6), R1.11 (τ/r estimator cost and scalability, §5.1 — the one item entirely absent before), and R2.7 (full-series percentile threshold = look-ahead in the target definition — disclosed in §6.8 + Threats(v); placement is target-free so only baseline reference numbers are touched; calibration-window threshold adopted as future work).
